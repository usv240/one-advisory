...
Judges can use the visual sandbox without authentication. Any developer can open **Developer API · 50/day**, generate a key without creating an account, and call the same governed managed-agent workflow through `/v1`. The key is displayed once; only an HMAC digest and keyed network fingerprint are stored. Firestore transactions atomically enforce 50 calls per key and originating network per UTC day.

Start with [DEVELOPER_API.md](DEVELOPER_API.md), the live `/docs` contract, or `GET /api/developer`. The public service accepts only fictional synthetic exercise data. Google recommends Secret Manager for Cloud Run secrets, so `API_KEY_PEPPER` is pinned to Secret Manager version 1: [Cloud Run secret configuration](https://docs.cloud.google.com/run/docs/configuring/services/secrets).

The judge-facing autonomy receipt is derived from the persisted timeline and managed Agent Runtime receipts. It reports cumulative automatic operations, protected incident-command decisions, external events, durable wakes, managed commands, and zero operator continue-clicks; `/v1/incidents/{incident_id}/autonomy-proof` exposes the classified receipt.
