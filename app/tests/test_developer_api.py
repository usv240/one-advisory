...
def test_key_is_one_time_hashed_and_quota_is_atomic():
    ...
    manager = DeveloperAccessManager(store, "test", "oa_live_", "test-pepper", now=lambda: datetime(2026, 8, 22, 12, tzinfo=timezone.utc))
...
def test_self_service_key_unlocks_real_v1_incident_workflow():
    client = TestClient(app)
    assert client.post("/v1/incidents", json=payload()).status_code == 401
    ...
    created = client.post("/v1/incidents", json=payload(), headers={"X-API-Key": key})
    assert created.status_code == 201
    assert created.headers["X-RateLimit-Remaining"] == "49"
    assert created.json()["origin"] == "pilot_input"
    assert created.json()["autonomy_proof"]["automatic_trace_events"] >= 1
